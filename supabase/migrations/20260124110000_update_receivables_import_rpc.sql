...
BEGIN
    ...
    INSERT INTO public.import_logs_receivables (
        company_id, user_id, file_name, total_rows, status, created_at
    ) VALUES (
        p_company_id, p_user_id, p_file_name, jsonb_array_length(p_rows), 'processing', now()
    ) RETURNING id INTO v_batch_id;

    -- 2. Clean Slate: Delete existing for company (Strict Replacement)
    ...
    LOOP
        ...
        -- Extract error first (passed from service sanitation)
        v_error_principal := trim(v_row->>'_error_principal');

        IF v_error_principal IS NOT NULL AND v_error_principal <> '' THEN
            v_reject_reason := v_error_principal;
        ELSE
            -- Extract & Parse

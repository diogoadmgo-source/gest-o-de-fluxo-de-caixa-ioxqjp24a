-- Migration to fix return type conflict and ensure correct table usage for dashboard functions
-- Drops existing functions to allow return type changes
-- Updates get_dashboard_kpis and get_latest_balances to use bank_balances_v2 which is the currently active table

-- 1. Fix get_latest_balances
DROP FUNCTION IF EXISTS public.get_latest_balances(uuid);
...
BEGIN
    ...
    FROM banks b
    LEFT JOIN bank_balances_v2 bb ON b.id = bb.bank_id
...
-- 2. Fix get_dashboard_kpis
DROP FUNCTION IF EXISTS public.get_dashboard_kpis(uuid, integer);
...
BEGIN
    ...
    -- 1. Current Balance (Latest known from bank_balances_v2)
...
-- 3. Fix get_cash_flow_aggregates (Ensure no changes in signature conflict, but updating just in case)
DROP FUNCTION IF EXISTS public.get_cash_flow_aggregates(uuid, date, date);
